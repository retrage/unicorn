 ...
 * Copyright (c) 2018 Akira Moroo, retrage01@gmail.com (Unicorn engine)
 *
...
static void generate_exception(DisasContext *ctx, int excp)
{
    ...
    tcg_gen_movi_tl(tcg_ctx, cpu_pc, ctx->pc);
...
static void generate_exception_mbadaddr(DisasContext *ctx, int excp)
{
    TCGContext *tcg_ctx = ctx->uc->tcg_ctx;
    tcg_gen_movi_tl(tcg_ctx, cpu_pc, ctx->pc);
    tcg_gen_st_tl(tcg_ctx, cpu_pc, tcg_ctx->cpu_env, offsetof(CPURISCVState, badaddr));
    TCGv_i32 helper_tmp = tcg_const_i32(tcg_ctx, excp);
    gen_helper_raise_exception(tcg_ctx, tcg_ctx->cpu_env, helper_tmp);
    tcg_temp_free_i32(tcg_ctx, helper_tmp);
    ctx->bstate = BS_BRANCH;
    ...
    uint32_t rs2, int rm, uint64_t min)
{
    ...
    case 0: /* fsgnj */
        if (rs1 == rs2) { /* FMOV */
            tcg_gen_mov_i64(tcg_ctx, cpu_fpr[rd], cpu_fpr[rs1]);
        } else {
            tcg_gen_deposit_i64(tcg_ctx, cpu_fpr[rd], cpu_fpr[rs2], cpu_fpr[rs1],
                                0, min == INT32_MIN ? 31 : 63);
        }
        break;
    case 1: /* fsgnjn */
        if (rs1 == rs2) { /* FNEG */
            tcg_gen_xori_i64(tcg_ctx, cpu_fpr[rd], cpu_fpr[rs1], min);
        } else {
            TCGv_i64 t0 = tcg_temp_new_i64(tcg_ctx);
            tcg_gen_not_i64(tcg_ctx, t0, cpu_fpr[rs2]);
            tcg_gen_deposit_i64(tcg_ctx, cpu_fpr[rd], t0, cpu_fpr[rs1],
                                0, min == INT32_MIN ? 31 : 63);
    ...
    case 2: /* fsgnjx */
        if (rs1 == rs2) { /* FABS */
            tcg_gen_andi_i64(tcg_ctx, cpu_fpr[rd], cpu_fpr[rs1], ~min);
        } else {
            TCGv_i64 t0 = tcg_temp_new_i64(tcg_ctx);
            tcg_gen_andi_i64(tcg_ctx, t0, cpu_fpr[rs2], min);
            tcg_gen_xor_i64(tcg_ctx, cpu_fpr[rd], cpu_fpr[rs1], t0);
            tcg_temp_free_i64(tcg_ctx, t0);
        }
        break;
    default:
        ...
        int rs2)
{
    ...
    case OPC_RISC_MULHSU:
        gen_mulhsu(tcg_ctx, source1, source1, source2);
        break;
    case OPC_RISC_MULHU:
        ...
        int rs1, target_long imm)
{
    ...
    gen_get_gpr(tcg_ctx, source1, rs1);
    ...
    gen_set_gpr(tcg_ctx, rd, source1);
    tcg_temp_free(tcg_ctx, source1);
}
                    ...
                    target_ulong imm)
{
    ...
    if (rd != 0) {
        tcg_gen_movi_tl(tcg_ctx, cpu_gpr[rd], ctx->next_pc);
    }
                     ...
                     int rd, int rs1, target_long imm)
{
    ...
    case OPC_RISC_JALR:
        ...
        tcg_gen_addi_tl(tcg_ctx, cpu_pc, cpu_pc, imm);
        tcg_gen_andi_tl(tcg_ctx, cpu_pc, cpu_pc, (target_ulong)-2);

        if (!riscv_has_ext(env, RVC)) {
            misaligned = gen_new_label(tcg_ctx);
            tcg_gen_andi_tl(tcg_ctx, t0, cpu_pc, 0x2);
                       ...
                       int rs1, int rs2, target_long bimm)
{
    ...
    source2 = tcg_temp_new(tcg_ctx);
    gen_get_gpr(tcg_ctx, source1, rs1);
    gen_get_gpr(tcg_ctx, source2, rs2);
        ...
        int rs1, target_long imm)
{
    ...
    switch (opc) {
    case OPC_RISC_FLW:
        tcg_gen_qemu_ld_i64(tcg_ctx, cpu_fpr[rd], t0, ctx->mem_idx, MO_TEUL);
        /* RISC-V requires NaN-boxing of narrower width floating point values */
        tcg_gen_ori_i64(tcg_ctx, cpu_fpr[rd], cpu_fpr[rd], 0xffffffff00000000ULL);
        break;
    case OPC_RISC_FLD:
        tcg_gen_qemu_ld_i64(tcg_ctx, cpu_fpr[rd], t0, ctx->mem_idx, MO_TEQ);
        break;
    default:
        gen_exception_illegal(ctx);
        break;
        ...
        int rs2, target_long imm)
{
    ...
    switch (opc) {
    case OPC_RISC_FSW:
        tcg_gen_qemu_st_i64(tcg_ctx, cpu_fpr[rs2], t0, ctx->mem_idx, MO_TEUL);
        break;
    case OPC_RISC_FSD:
        tcg_gen_qemu_st_i64(tcg_ctx, cpu_fpr[rs2], t0, ctx->mem_idx, MO_TEQ);
        break;
    default:
        gen_exception_illegal(ctx);
        break;
                      ...
                      int rd, int rs1, int rs2)
{
    ...
    case OPC_RISC_LR:
        ...
        tcg_gen_qemu_ld_tl(tcg_ctx, load_val, src1, ctx->mem_idx, mop);
        if (aq) {
            tcg_gen_mb(tcg_ctx, TCG_MO_ALL | TCG_BAR_LDAQ);
        }
        tcg_gen_mov_tl(tcg_ctx, load_res, src1);
        gen_set_gpr(tcg_ctx, rd, load_val);
        break;
    ...
    case OPC_RISC_SC:
        ...
        gen_get_gpr(tcg_ctx, src1, rs1);
        tcg_gen_brcond_tl(tcg_ctx, TCG_COND_NE, load_res, src1, l1);

        gen_get_gpr(tcg_ctx, src2, rs2);
        /* Note that the TCG atomic primitives are SC,
           so we can ignore AQ/RL along this path.  */
        tcg_gen_atomic_cmpxchg_tl(tcg_ctx, src1, load_res, load_val, src2,
                                  ctx->mem_idx, mop);
        tcg_gen_setcond_tl(tcg_ctx, TCG_COND_NE, dat, src1, load_val);
        gen_set_gpr(tcg_ctx, rd, dat);
    ...
    do_minmax:
        ...
        if ((mop & MO_SSIZE) == MO_SL) {
            ...
            tcg_gen_ext32s_tl(tcg_ctx, src2, src2);
        }
        dat = tcg_temp_local_new(tcg_ctx);
        tcg_gen_qemu_ld_tl(tcg_ctx, dat, src1, ctx->mem_idx, mop);
        tcg_gen_movcond_tl(tcg_ctx, cond, src2, dat, src2, dat, src2);

        if (tb_cflags(ctx->tb) & CF_PARALLEL) {
            /* Parallel context.  Make this operation atomic by verifying
               that the memory didn't change while we computed the result.  */
            tcg_gen_atomic_cmpxchg_tl(tcg_ctx, src2, src1, dat, src2, ctx->mem_idx, mop);
            ...
            tcg_gen_brcond_tl(tcg_ctx, TCG_COND_NE, dat, src2, l1);
        } else {
            /* Serial context.  Directly store the result.  */
            tcg_gen_qemu_st_tl(tcg_ctx, src2, src1, ctx->mem_idx, mop);
        }
        gen_set_gpr(tcg_ctx, rd, dat);
        tcg_temp_free(tcg_ctx, dat);
        break;
                         ...
                         int rs1, int rs2, int rs3, int rm)
{
    ...
    case OPC_RISC_FMADD_S:
        gen_set_rm(ctx, rm);
        gen_helper_fmadd_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1],
                           cpu_fpr[rs2], cpu_fpr[rs3]);
        break;
    case OPC_RISC_FMADD_D:
        gen_set_rm(ctx, rm);
        gen_helper_fmadd_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1],
                           cpu_fpr[rs2], cpu_fpr[rs3]);
        break;
    default:
        gen_exception_illegal(ctx);
                         ...
                         int rs1, int rs2, int rs3, int rm)
{
    ...
    case OPC_RISC_FMSUB_S:
        gen_set_rm(ctx, rm);
        gen_helper_fmsub_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1],
                           cpu_fpr[rs2], cpu_fpr[rs3]);
        break;
    case OPC_RISC_FMSUB_D:
        gen_set_rm(ctx, rm);
        gen_helper_fmsub_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1],
                           cpu_fpr[rs2], cpu_fpr[rs3]);
        break;
    default:
        gen_exception_illegal(ctx);
                          ...
                          int rs1, int rs2, int rs3, int rm)
{
    ...
    case OPC_RISC_FNMSUB_S:
        gen_set_rm(ctx, rm);
        gen_helper_fnmsub_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1],
                            cpu_fpr[rs2], cpu_fpr[rs3]);
        break;
    case OPC_RISC_FNMSUB_D:
        gen_set_rm(ctx, rm);
        gen_helper_fnmsub_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1],
                            cpu_fpr[rs2], cpu_fpr[rs3]);
        break;
    default:
        gen_exception_illegal(ctx);
                          ...
                          int rs1, int rs2, int rs3, int rm)
{
    ...
    case OPC_RISC_FNMADD_S:
        gen_set_rm(ctx, rm);
        gen_helper_fnmadd_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1],
                            cpu_fpr[rs2], cpu_fpr[rs3]);
        break;
    case OPC_RISC_FNMADD_D:
        gen_set_rm(ctx, rm);
        gen_helper_fnmadd_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1],
                            cpu_fpr[rs2], cpu_fpr[rs3]);
        break;
    default:
        gen_exception_illegal(ctx);
                         ...
                         int rs1, int rs2, int rm)
{
    ...
    case OPC_RISC_FADD_S:
        gen_set_rm(ctx, rm);
        gen_helper_fadd_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
        break;
    case OPC_RISC_FSUB_S:
        gen_set_rm(ctx, rm);
        gen_helper_fsub_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
        break;
    case OPC_RISC_FMUL_S:
        gen_set_rm(ctx, rm);
        gen_helper_fmul_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
        break;
    case OPC_RISC_FDIV_S:
        gen_set_rm(ctx, rm);
        gen_helper_fdiv_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
        break;
    case OPC_RISC_FSQRT_S:
        gen_set_rm(ctx, rm);
        gen_helper_fsqrt_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1]);
        break;
    ...
    case OPC_RISC_FMIN_S:
        ...
        switch (rm) {
        case 0x0:
            gen_helper_fmin_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        case 0x1:
            gen_helper_fmax_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FEQ_S:
        ...
        switch (rm) {
        case 0x0:
            gen_helper_fle_s(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        case 0x1:
            gen_helper_flt_s(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        case 0x2:
            gen_helper_feq_s(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        default:
            goto do_illegal;
        }
        gen_set_gpr(tcg_ctx, rd, t0);
        tcg_temp_free(tcg_ctx, t0);
        break;
    ...
    case OPC_RISC_FCVT_W_S:
        ...
        case 0: /* FCVT_W_S */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_w_s(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
        case 1: /* FCVT_WU_S */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_wu_s(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
#if defined(TARGET_RISCV64)
        case 2: /* FCVT_L_S */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_l_s(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
        case 3: /* FCVT_LU_S */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_lu_s(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
#endif
        default:
            goto do_illegal;
    ...
    case OPC_RISC_FCVT_S_W:
        ...
        case 0: /* FCVT_S_W */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_w(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
        case 1: /* FCVT_S_WU */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_wu(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
#if defined(TARGET_RISCV64)
        case 2: /* FCVT_S_L */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_l(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
        case 3: /* FCVT_S_LU */
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_lu(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
#endif
        default:
            goto do_illegal;
    ...
    case OPC_RISC_FMV_X_S:
        ...
        case 0: /* FMV */
#if defined(TARGET_RISCV64)
            tcg_gen_ext32s_tl(tcg_ctx, t0, cpu_fpr[rs1]);
#else
            tcg_gen_extrl_i64_i32(tcg_ctx, t0, cpu_fpr[rs1]);
#endif
            break;
        case 1:
            gen_helper_fclass_s(tcg_ctx, t0, cpu_fpr[rs1]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FMV_S_X:
        ...
        gen_get_gpr(tcg_ctx, t0, rs1);
#if defined(TARGET_RISCV64)
        tcg_gen_mov_i64(tcg_ctx, cpu_fpr[rd], t0);
#else
        tcg_gen_extu_i32_i64(tcg_ctx, cpu_fpr[rd], t0);
#endif
        tcg_temp_free(tcg_ctx, t0);
        break;
    ...
    case OPC_RISC_FADD_D:
        gen_set_rm(ctx, rm);
        gen_helper_fadd_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
        break;
    case OPC_RISC_FSUB_D:
        gen_set_rm(ctx, rm);
        gen_helper_fsub_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
        break;
    case OPC_RISC_FMUL_D:
        gen_set_rm(ctx, rm);
        gen_helper_fmul_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
        break;
    case OPC_RISC_FDIV_D:
        gen_set_rm(ctx, rm);
        gen_helper_fdiv_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
        break;
    case OPC_RISC_FSQRT_D:
        gen_set_rm(ctx, rm);
        gen_helper_fsqrt_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1]);
        break;
    ...
    case OPC_RISC_FMIN_D:
        ...
        switch (rm) {
        case 0:
            gen_helper_fmin_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        case 1:
            gen_helper_fmax_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FCVT_S_D:
        ...
        case 1:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_s_d(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FCVT_D_S:
        ...
        case 0:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_s(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FEQ_D:
        ...
        switch (rm) {
        case 0:
            gen_helper_fle_d(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        case 1:
            gen_helper_flt_d(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        case 2:
            gen_helper_feq_d(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1], cpu_fpr[rs2]);
            break;
        default:
            goto do_illegal;
        }
    ...
    case OPC_RISC_FCVT_W_D:
        ...
        case 0:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_w_d(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
        case 1:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_wu_d(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
#if defined(TARGET_RISCV64)
        case 2:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_l_d(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
        case 3:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_lu_d(tcg_ctx, t0, tcg_ctx->cpu_env, cpu_fpr[rs1]);
            break;
#endif
        default:
            goto do_illegal;
    ...
    case OPC_RISC_FCVT_D_W:
        ...
        case 0:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_w(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
        case 1:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_wu(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
#if defined(TARGET_RISCV64)
        case 2:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_l(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
        case 3:
            gen_set_rm(ctx, rm);
            gen_helper_fcvt_d_lu(tcg_ctx, cpu_fpr[rd], tcg_ctx->cpu_env, t0);
            break;
#endif
        default:
            goto do_illegal;
    ...
    case OPC_RISC_FMV_X_D:
        ...
        switch (rm) {
        case 0: /* FMV */
            gen_set_gpr(tcg_ctx, rd, cpu_fpr[rs1]);
            break;
        case 1:
            t0 = tcg_temp_new(tcg_ctx);
    ...
    case OPC_RISC_FMV_D_X:
        t0 = tcg_temp_new(tcg_ctx);
        gen_get_gpr(tcg_ctx, t0, rs1);
        tcg_gen_mov_tl(tcg_ctx, cpu_fpr[rd], t0);
        tcg_temp_free(tcg_ctx, t0);
        break;
#endif
                      ...
                      int rd, int rs1, int csr)
{
    ...
    case OPC_RISC_ECALL:
        ...
        case 0x0: /* ECALL */
            ...
            tcg_gen_exit_tb(tcg_ctx, 0); /* no chaining */
            ctx->bstate = BS_BRANCH;
            break;
        case 0x1: /* EBREAK */
        ...
        case 0x102: /* SRET */
            if (riscv_has_ext(env, RVS)) {
                gen_helper_sret(tcg_ctx, cpu_pc, tcg_ctx->cpu_env, cpu_pc);
                tcg_gen_exit_tb(tcg_ctx, 0); /* no chaining */
                ctx->bstate = BS_BRANCH;
            } else {
                gen_exception_illegal(ctx);
        ...
        case 0x202: /* HRET */
            ...
            break;
        case 0x302: /* MRET */
            gen_helper_mret(tcg_ctx, cpu_pc, tcg_ctx->cpu_env, cpu_pc);
            tcg_gen_exit_tb(tcg_ctx, 0); /* no chaining */
            ctx->bstate = BS_BRANCH;
            break;
        case 0x7b2: /* DRET */
            ...
            break;
        case 0x105: /* WFI */
            tcg_gen_movi_tl(tcg_ctx, cpu_pc, ctx->next_pc);
...
static void decode_RV32_64C1(CPURISCVState *env, DisasContext *ctx)
{
    ...
    case 3:
        if (rd_rs1 == 2) {
            ...
            tcg_gen_movi_tl(tcg_ctx, cpu_gpr[rd_rs1],
...
static void decode_RV32_64G(CPURISCVState *env, DisasContext *ctx)
{
    ...
    case OPC_RISC_LUI:
        ...
        tcg_gen_movi_tl(tcg_ctx, cpu_gpr[rd], sextract64(ctx->opcode, 12, 20) << 12);
        break;
    case OPC_RISC_AUIPC:
        if (rd == 0) {
            break; /* NOP */
...
void riscv_tcg_init(struct uc_struct *uc)
{
    ...
    cpu_pc = tcg_global_mem_new(tcg_ctx, TCG_AREG0,
            offsetof(CPURISCVState, pc), "pc");
    load_res = tcg_global_mem_new(tcg_ctx, TCG_AREG0, 
            offsetof(CPURISCVState, load_res), "load_res");
    load_val = tcg_global_mem_new(tcg_ctx, TCG_AREG0, 
            offsetof(CPURISCVState, load_val), "load_val");
}
