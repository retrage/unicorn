...
static void riscv32_set_pc(struct uc_struct *uc, uint32_t address)
{
    ((CPURISCVState *)uc->current_cpu->env_ptr)->pc = address;
}

void riscv32_release(void *ctx);
void riscv32_release(void *ctx)
{
    struct uc_struct *uc;
    RISCVCPU *cpu;
...
int riscv32_reg_read(struct uc_struct *uc, unsigned int *regs, void **vals, int count)
{
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (regid >= UC_RISCV_REG_X0 && regid <= UC_RISCV_REG_X31)
            *(uint32_t *)value = RISCV_CPU(uc, mycpu)->env.gpr[regid - UC_RISCV_REG_X0];
            ...
            *(uint64_t *)value = RISCV_CPU(uc, mycpu)->env.fpr[regid - UC_RISCV_REG_F0];
            ...
            switch(regid) {
                ...
                case UC_RISCV_REG_PC:
                         *(uint32_t *)value = RISCV_CPU(uc, mycpu)->env.pc;
                         break;
            }
        }
    }
...
int riscv32_reg_write(struct uc_struct *uc, unsigned int regs, void *const *vals, int count)
{
    ...
    for (i = 0; i < count; i++) {
        ...
        if (regid >= UC_RISCV_REG_X0 && regid <= UC_RISCV_REG_X31)
            RISCV_CPU(uc, mycpu)->env.gpr[regid - UC_RISCV_REG_X0] = *(uint32_t *)value;
        else if (regid >= UC_RISCV_REG_F0 && regid <= UC_RISCV_REG_F31)
            ...
            switch(regid) {
                default: break;
                case UC_RISCV_REG_PC:
                         RISCV_CPU(uc, mycpu)->env.pc = *(uint32_t *)value;
...
void riscv32_uc_init(struct uc_struct *uc)
{
    ...
    riscv32_cpu_register_types(uc);
    uc->reg_read = riscv32_reg_read;
    uc->reg_write = riscv32_reg_write;
    uc->reg_reset = riscv32_reg_reset;
    uc->set_pc = riscv32_set_pc;
    uc->release = riscv32_release;
    uc_common_init(uc);
}
